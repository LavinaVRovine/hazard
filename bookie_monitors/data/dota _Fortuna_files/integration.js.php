iapiConf = {"casinoname":"bifortuna.cz","loginServer":"login.ifortuna.cz","clientSkin":"bifortuna.cz","clientType":"casino","clientPlatform":"web","clientVersion":null,"systemId":"868","serviceType":"GamePlay","loginDomainRetryCount":"2","loginDomainRequestTimeout":"30","loginDomainRetryInterval":"1","fingerprintEnabled":"0","onlypostrequestsforlogout":null,"useIframeForGetLoggedInPlayer":"0","syncDone":"0","syncHops":5,"clientUrl_casino":"http:\/\/cachedownload-b.ifortuna.cz\/casinoclient.html"};
...
function pickyBrowserCookieSync()
{
  ...
  if (!inIframe() && (iapiConf['safariSyncDisabled'] == -1 || !iapiConf['loginServer'].endsWith("."+getCurrentDomain())))//-1 forces; iapiConf['loginServer'].endsWith("."+getCurrentDomain() <-- subdomain check
  {
    ...
    if (syncHops > 0 && (!self.$domainSync || self.$domainSync.indexOf(iapiConf['loginServer'])<0))//not sync'd already
    {
      ...
      var back = new URL("/DomainSync.php", "https://" + iapiConf['loginServer']);
