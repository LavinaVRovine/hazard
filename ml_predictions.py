...
def try_mxnet(db_loc):
    ...
    X_test = mx.nd.crop(
        X, begin=(train_count, 0), end=(sample_count, feature_count - 1)
    )
    ...
    test = mx.gluon.data.DataLoader(
        mx.gluon.data.ArrayDataset(X_test, y_test), batch_size=batch_size, shuffle=False
    )
...
if __name__ == "__main__":
    ...
    df = df.reindex(
        columns=(
            ["main_team_won"] + list([a for a in df.columns if a != "main_team_won"])
        )
    )
    pass
